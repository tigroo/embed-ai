...
def run(model: str, output_dir: str, summary_file: str, pwa_only: bool = False):
    ...
    if pwa_only:
        ...
        if bench_cache.exists():
            ...
            all_video_results = benchmark.bench(model_list=model_list, source_paths=source_paths,
                output_root=output_root, torch_gpu_ok=torch_gpu_ok, )
            bench_cache.write_text(json.dumps(all_video_results, indent=2))
            logger.info("Benchmark results cached to %s", bench_cache)

        # ── Summary ──────────────────────────────────────────────────────
        summary.summarize(model_list=model_list, source_paths=source_paths, output_root=output_root,
            summary_file=summary_file, cal_yaml=cal_yaml, cal_diag=cal_diagnostic, eval_results=eval_results,
            all_video_results=all_video_results, torch_gpu_ok=torch_gpu_ok, )
...
if __name__ == "__main__":
    ...
    run(model=args.model, output_dir=args.output, summary_file=args.summary, pwa_only=args.pwa_only, )
