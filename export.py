...
def export_tflite(
    model: str, output_root: Path, calibration_yaml: str, task: str = "detect"
) -> dict[str, str]:
    ...
    need_float = (
        not Path(model_list["fp32"]).exists() or not Path(model_list["fp16"]).exists()
    )
    need_int8 = (
    ...
    if need_float or need_int8:
        if torch.cuda.is_available():
            ...
            logger.info(
                "PyTorch CUDA context pinned (%s)", torch.cuda.get_device_name(0)
            )
        ...
        try:
            ...
            if task == "detect" and need_int8:
                ...
                if calibration_yaml and Path(calibration_yaml).exists():
                    ...
                    logger.info(
                        f"Exporting TFLite int8 (dynamic + integer + full) with custom calibration: {_INT8_CAL_DATA}"
                    )
                else:
                    _INT8_CAL_DATA = "coco128.yaml"
                    logger.info(
                        "Exporting TFLite int8 (dynamic + integer + full) with default calibration: coco128.yaml"
                    )
                logger.info(
...
def export_onnx_for_pwa(output_root: Path, model: str) -> tuple[Path, Path]:
    ...
    if pwa_fp32.exists():
        logger.info(
            "FP32 ONNX for PWA model already present: %s (%.1f MB)",
            pwa_fp32,
            file_size_mb(pwa_fp32),
        )
    else:
    ...
    if pwa_quant.exists():
        logger.info(
            "Quantized ONNX for PWA model already present: %s (%.1f MB)",
            pwa_quant,
            file_size_mb(pwa_quant),
        )
    else:
