...
def evaluate(model_list: dict[str, str]) -> list[dict]:
    ...
    for tag, model_path in model_list.items():
        ...
        if not p.exists():
            ...
            continue
        logger.info("Evaluating %-8s (%s) on %s ...", tag, p.name, _EVAL_DATA)
        model = YOLO(str(p), task="detect")
        try:
            metrics = model.val(data=_EVAL_DATA, imgsz=TFLITE_IMG_SIZE, verbose=False)
            entry = {
                "tag": tag,
                "model_path": str(p),
